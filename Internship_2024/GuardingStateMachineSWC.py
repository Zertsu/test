...
global check_entry
check_entry = False # this checks if we already entered in shooting mode
...
emergency_distance = 30 # minimum distance from obsticle
...
max_eye_margin = 25 # maximum distance between the eyes
...
def shooting():
    ...
    if face_position[0] == enemy:
        ...
        if check_entry == False:
            state = States.SHOOT
            Rte_Write_GuardingStateMachineSWC_E_State(state)
        shooting_bit = Rte_Read_GuardingStateMachineSWC_b_Shoot()
        if shooting_bit == False and check_entry == True:
