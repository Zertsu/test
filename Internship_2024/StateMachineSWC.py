...
from typedefs import States, States_STR
...
# Logger
import Logger
log = Logger.Logger("State Machine")
...
def checkTranitionNeeded_Idle():
    ...
    # Change state acording to the control bits
        ...
        log.LOGI("Entering guarding mode")
...
def guardingModeHandler():
    ...
    if timeoutEmergency or distanceEmergency:
        if timeoutEmergency:
            log.LOGW("Recieved timeout emergency in guarding mode, exiting guarding mode")
        if distanceEmergency:
            log.LOGW("Recieved distance emergency in guarding mode, exiting guarding mode")
        guardingMode = False
    ...
    if guardingBit and not previousGuardingBit:
        ...
        log.LOGI("User requested exit from guarding mode, exiting guarding mode")
        guardingMode = False
        Rte_Write_StateMachineSWC_b_guarding_mode(guardingMode)
    previousGuardingBit = guardingBit
...
async def state_machine():
    ...
    log.LOGI("Starting StateMachineSWC")
    while True:
        ...
        if not guardingState:
            ...
            if nextState != state:
                log.LOGI("Changeing state from {} to {}".format(States_STR[state], States_STR[nextState]))
